/**
 * 酷狗音乐 API 行为指纹模拟生成器（Node.js 版）
 *
 * 生成模拟的用户行为指纹数据，用于绕过 SSA 二次验证：
 * 1. 生成模拟鼠标移动轨迹（贝塞尔曲线 + 随机抖动）
 * 2. 生成模拟页面交互事件（滚动、窗口 resize 等）
 * 3. AES-128-CBC 加密行为数据得到 EDT
 * 4. RSA-OAEP SHA-256 加密 AES 密钥得到 SID
 */
const { randomString, generateWebGLHash } = require('./util');
const CryptoJS = require('crypto-js');
const forge = require('node-forge');

/**
 * RSA 公钥，用于 RSA-OAEP SHA-256 加密 AES 密钥
 */
const publicKey = `-----BEGIN PUBLIC KEY-----\\nrQIDAQAB\n-----END PUBLIC KEY-----`;

/**
 * AES 初始化向量（固定值 "kugousecurity123"）
 * @type {string}
 */
const iv = 'kugousecurity123';

/**
 * 哨兵值（接近 0xFFFFFFFF 的随机值），每次调用重新生成
 * @type {number}
 */
let SENTINEL = 0xffffffff - Math.floor(Math.random() * 20);

/**
 * 生成 [min, max] 范围内的随机整数（包含两端）
 * @param {number} min
 * @param {number} max
 * @returns {number}
 */
function ri(min, max) {
  return Math.floor(Math.random() * (max - min + 1)) + min;
}

// ============================================================
// 事件记录格式化函数
// ============================================================

function f3(t, i, x, y) {
  return `3,${t},${i},${x},${y}`;
}
function f5(t, i) {
  return `5,${t},${i}`;
}
function f6(t, i, x, y) {
  return `6,${t},${i},${x},${y}`;
}
function fs3(i, x, y) {
  return `3,${SENTINEL},${i},${x},${y}`;
}
function fs5(i) {
  return `5,${SENTINEL},${i}`;
}
function fs6(i, x, y) {
  return `6,${SENTINEL},${i},${x},${y}`;
}

// ============================================================
// 贝塞尔曲线鼠标路径生成
// ============================================================

/**
 * 用三阶贝塞尔曲线生成模拟真人的鼠标移动路径
 * @param {number} sx - 起点 X
 * @param {number} sy - 起点 Y
 * @param {number} ex - 终点 X
 * @param {number} ey - 终点 Y
 * @param {number} n - 采样点数
 * @returns {Array<{x:number, y:number}>}
 */
function bezierPath(sx, sy, ex, ey, n) {
  const c1x = sx + (ex - sx) * 0.3 + ri(-80, 80);
  const c1y = sy + (ey - sy) * 0.2 + ri(-60, 60);
  const c2x = sx + (ex - sx) * 0.7 + ri(-60, 60);
  const c2y = sy + (ey - sy) * 0.8 + ri(-40, 40);
  const pts = [];
  for (let i = 0; i <= n; i++) {
    const t = i / n;
    const u = 1 - t;
    const x = u * u * u * sx + 3 * u * u * t * c1x + 3 * u * t * t * c2x + t * t * t * ex;
    const y = u * u * u * sy + 3 * u * u * t * c1y + 3 * u * t * t * c2y + t * t * t * ey;
    const jitter = Math.max(0.5, 3 - t * 2.5);
    pts.push({
      x: x + (Math.random() - 0.5) * jitter,
      y: y + (Math.random() - 0.5) * jitter,
    });
  }
  return pts;
}

/**
 * 生成 EDT 中的 data 字段（用户行为指纹数据）
 * @param {Object} opts
 * @param {number} opts.startX
 * @param {number} opts.startY
 * @param {number} opts.endX
 * @param {number} opts.endY
 * @param {number} opts.mousePoints
 * @returns {string}
 */
function generateEDTData(opts) {
  const { startX, startY, endX, endY, mousePoints } = opts;
  const entries = [];
  let ts = 0;
  let ei = 0;

  // 初始化两个 type-5 零事件
  entries.push(f5(0, 0));
  entries.push(fs5(0));
  entries.push(f5(0, 0));
  entries.push(fs5(0));

  // 窗口事件 (type 6) - 模拟移动端 750x500
  ts += ri(5, 20);
  entries.push(f6(ts, ei, 750, 500));
  entries.push(fs6(ei, 750, 500));
  ei++;

  // 滚动事件 (type 5) - 3 次
  for (let i = 0; i < 3; i++) {
    ts += ri(80, 600);
    entries.push(f5(ts, ei));
    entries.push(fs5(ei));
    ei++;
  }

  // 鼠标轨迹 (type 3) - 贝塞尔曲线
  const path = bezierPath(startX, startY, endX, endY, mousePoints);
  let si = 0;
  for (let i = 0; i < path.length; i++) {
    const { x, y } = path[i];
    ts += ri(8, 50);
    entries.push(f3(ts, si, Math.round(x), Math.round(y)));
    entries.push(fs3(si, Math.round(x), Math.round(y)));
    if (i > 0 && i % 12 === 0) {
      ts += ri(20, 60);
      entries.push(f5(ts, ei));
      entries.push(fs5(ei));
      ei++;
    }
    si = (si + 1) % 2;
  }

  // 结束事件 - 最后位置带微调
  ts += ri(5, 30);
  entries.push(f3(ts, 1, Math.round(endX + ri(-5, 5)), Math.round(endY + ri(-5, 5))));
  entries.push(fs3(1, Math.round(endX), Math.round(endY)));

  return entries.join(':');
}

// ============================================================
// 核心导出函数
// ============================================================

/**
 * 生成模拟的 sid 和 edt 加密数据
 *
 * 流程：
 * 1. 生成随机 AES-128 密钥
 * 2. 生成模拟行为数据（鼠标轨迹 + 滚动 + 窗口事件）
 * 3. 拼接明文: mid=xxx;userid=xxx;dfid=xxx;webgl=xxx;webdriver=0;ts=xxx;data=xxx
 * 4. AES-128-CBC 加密明文 → EDT（Base64）
 * 5. RSA-OAEP SHA-256 加密 AES 密钥 → SID（Base64）
 *
 * @param {string|number} mid - 设备 MID 标识
 * @param {string|number} userid - 用户 ID
 * @param {string|number} dfid - 设备指纹 ID
 * @param {string} [webglHash] - WebGL 指纹哈希
 * @returns {{ edt: string, sid: string }}
 */
const generateSimulate = (mid, userid, dfid, webglHash) => {
  SENTINEL = 0xffffffff - Math.floor(Math.random() * 20);

  // 生成随机 AES-128 密钥（16 字符）
  const key = CryptoJS.MD5(randomString(16)).toString(CryptoJS.enc.Hex).substring(0, 16);

  // 随机化鼠标轨迹参数
  const points = ri(30, 60);
  const startX = ri(200, 600);
  const startY = ri(200, 500);
  const endX = ri(500, 700);
  const endY = ri(80, 150);

  mid = mid || 0;
  userid = userid || 0;
  dfid = dfid || 0;
  webglHash = webglHash || generateWebGLHash();
  const ts = Date.now();

  // 生成行为数据
  const data = generateEDTData({ startX, startY, endX, endY, mousePoints: points });

  // 拼接明文
  const sidPlaintext = `mid=${mid};userid=${userid};dfid=${dfid};webgl=${webglHash};webdriver=0;ts=${ts};data=${data}`;

  // Step 1: AES-128-CBC 加密 → EDT
  const edtData = CryptoJS.AES.encrypt(sidPlaintext, CryptoJS.enc.Utf8.parse(key), {
    iv: CryptoJS.enc.Utf8.parse(iv),
    mode: CryptoJS.mode.CBC,
    padding: CryptoJS.pad.Pkcs7,
  }).toString();

  // Step 2: RSA-OAEP SHA-256 加密 AES 密钥 → SID
  const rsaKey = forge.pki.publicKeyFromPem(publicKey);
  const encrypted = rsaKey.encrypt(key, 'RSA-OAEP', {
    md: forge.md.sha256.create(),
    mgf1: { md: forge.md.sha256.create() },
  });
  const ciphertext = forge.util.encode64(encrypted);

  return { edt: edtData, sid: ciphertext };
};

module.exports = { generateSimulate };
